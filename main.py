...
class Screen:
    ...
    def draw_sensors(self):
        ...
        if self.sensor_network.base_station:
            x, y = self.sensor_network.base_station.position
            screen_x = int(round((x / 1000) * self.screen.get_width()))
            screen_y = int(round((y / 1000) * self.screen.get_height()))
            pygame.draw.circle(self.screen, (255, 0, 0), (screen_x, screen_y), 8)
    
    def draw_communication_lines(self):
        pairs = self.sensor_network.get_communication_pairs()
        for sensor1, sensor2 in pairs:
...
if __name__ == "__main__":
    ...
    visualizer = Screen(width=800, height=800, bg_color=(255, 255, 255), sensor_network=sensor_network)
    visualizer.run()